...
<div
    class="fixed bottom-0 left-0 right-0 border-t-4 border-black bg-white z-50 pb-safe"
>
    <div class="flex items-stretch h-20">
        {#each tabs as tab, index}
            ...
            >
                <Icon
                    class="w-6 h-6 {isActive ? 'stroke-[2.5px]' : 'stroke-2'}"
                />
                <span
                ...
                {#if isActive}
                    <div
                        class="absolute top-0 left-0 right-0 h-1 bg-black"
                    ></div>
                {/if}
            </button>
        {/each}
